...
const ManualBookingModal: React.FC<ManualBookingModalProps> = React.memo(({
    ...
    return createPortal(
        <div className="fixed inset-0 z-50 overflow-y-auto flex justify-center items-start md:items-center p-4 md:p-6 bg-charcoal/80 backdrop-blur-sm">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl relative my-auto max-h-[90vh] flex flex-col overflow-hidden">
                <div className="sticky top-0 z-10 bg-white px-6 md:px-8 pt-6 md:pt-8 pb-4 flex justify-between items-center border-b border-gold/10">
                    <h2 className="font-serif text-xl md:text-2xl text-charcoal">New Guest Reservation</h2>
                    <button onClick={onClose} className="text-charcoal/40 hover:text-gold flex-shrink-0">
                        <XCircle size={24} />
                    </button>
                </div>
