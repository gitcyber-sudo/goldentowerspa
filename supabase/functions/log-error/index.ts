...
serve(async (req) => {
    ...
    try {
        ...
        // 1. Check for recent duplicates to prevent DB and email spam
        // We only log to DB and send an email if this specific error hasn't been logged in the last hour
        ...
        if (recentLogs && recentLogs.length > 0) {
            console.log('Duplicate error suppressed (within 1 hour):', message);
        ...
        if (resendApiKey) {
            const res = await fetch('https://api.resend.com/emails', {
                ...
                body: JSON.stringify({
                    from: 'Golden Tower Spa <onboarding@resend.dev>',
                    ...
                    html: `
                        ...
                        <p style="font-size: 10px; color: #666;">
                            ...
                            Identical errors occurring within the next hour will be completely silenced and not logged to the database.
                        </p>
                    `
                }),
            });
