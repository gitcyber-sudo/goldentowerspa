...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Updating password for therapist_id: ${therapist_id}, user_id: ${targetUserId}`)
    ...
    if (updateError) {
      console.error('Update User Auth Error:', updateError)
      throw updateError
    }

    console.log(`Password successfully updated for user_id: ${targetUserId}`)
